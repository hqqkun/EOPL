      ...
      (lambda (new-rands)
        (cases simple-expression k-exp
          (cps-proc-exp (vars body)
            (let ([new-k (fresh-identifier 'k)])
              (cps-let-exp new-k k-exp
                (cps-if-exp 
                  (car new-rands)
                  (cps-of-exp exp2 (cps-var-exp new-k))
                  (cps-of-exp exp3 (cps-var-exp new-k))))))
          (cps-var-exp (var)
            (cps-if-exp
              (car new-rands)
              (cps-of-exp exp2 k-exp)
              (cps-of-exp exp3 k-exp)))
          (else (eopl:error "no matching for k-exp : ~s" k-exp))))))
)
