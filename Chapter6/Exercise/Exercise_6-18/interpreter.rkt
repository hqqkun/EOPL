  ...
  (lambda (exp env)
    (cases simple-expression exp
      ...
      (cps-diff-exp (exp1 exp2)
        (let
          ( [num1 (expval->num (value-of-const-or-var exp1 env))]
            [num2 (expval->num (value-of-const-or-var exp2 env))])
          (num-val (- num1 num2))))
      (cps-zero?-exp (exp1)
        (let ([num1 (expval->num (value-of-const-or-var exp1 env))])
          (bool-val (zero? num1))))
      (cps-sum-exp (exps)
        (let
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; Exercise 6-18
(define value-of-const-or-var
  (lambda (exp env)
    (cases const-or-var exp
...
(define str "let p = proc(x y) -(x, y) in (p 1 2)")
(define str-1 "let x = 4 in if zero?(0) then 1 else 3")
(define run
  (lambda (str)
    (display 
