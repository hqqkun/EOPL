...
(define value-of/k
  (lambda (exp env cont)
    (cases expression exp
      ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
      (make-list-exp (exps)
        (value-of-explist exps env cont))
...
(define apply-cont
  (lambda (cont val)
    (cases continuation cont
      ;; list
      ;;;;;;;;;;;;;;;;;;;;;;;;;;
      (rest-cont (exps saved-env saved-cont)
        (value-of-explist exps saved-env
          (make-pair-cont val saved-cont)))
      
      (make-pair-cont (val1 saved-cont)
...
; value-of-explist : ListOf(Exps) * Env * Cont -> FinalAnswer
(define value-of-explist
  (lambda (exps env cont)
    (if (null? exps)
      (apply-cont cont (empty-val))
      (value-of/k (car exps) env
        (rest-cont (cdr exps) env cont))))
)