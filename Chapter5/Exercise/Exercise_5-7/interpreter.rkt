  ...
  (lambda (exp env cont)
    (cases expression exp
      ...
      (let-exp (vars exps body)
        (value-of-let/k vars exps body env cont))
...
(define apply-cont
  (lambda (cont val)
    (cases continuation cont
      ...
      (let-exp-cont (ps vars exps body saved-env saved-cont)
        (let ([new-ps (cons (cons (car vars) val) ps)])
          (if (null? exps)
            (value-of/k body (extend-env* (reverse new-ps) saved-env) saved-cont)
            (value-of/k (car exps) saved-env
              (let-exp-cont 
                new-ps
                (cdr vars) (cdr exps) body
