...
(provide initialize-scheduler! set-final-answer!
  ...
  run-next-thread get-time-remaining)
...
(define place-on-ready-queue!
  (lambda (th time-remain)
    (set! the-ready-queue
      (enqueue the-ready-queue (cons th time-remain))))
)
  ...
  (lambda ()
    (if (empty? the-ready-queue)
      ...
      (dequeue the-ready-queue
        (lambda (first-ready-thread-pair other-ready-threads)
          (set! the-ready-queue other-ready-threads)
          (let ([time-remain (cdr first-ready-thread-pair)])
            (if (zero? time-remain)
              (set! the-time-remaining the-max-time-slice)
              (set! the-time-remaining time-remain)))
          ((car first-ready-thread-pair))))))
)
