    ...
    (cases expression exp
      ...
      (deref-exp (exp1)
        (let ( [item-ty (fresh-tvar-type)])
          (cases answer (type-of exp1 tenv subst)
            (an-answer (ty1 subst)
              (let ( [subst (unifier ty1 (ref-type item-ty) subst exp1)])
                (an-answer item-ty subst))))))
        ...
        (cases answer (type-of exp1 tenv subst)
          (an-answer (ty1 subst)
            (cases answer (type-of exp2 tenv subst)
              (an-answer (ty2 subst)
                (let ([subst (unifier ty1 (ref-type ty2) subst exp1)])
                  (an-answer (void-type) subst)))))))
      
      (begin-exp (exp1 exps)
        (let ([ans (type-of exp1 tenv subst)])
          (let loop ([exps exps] [ans ans])
            (if (null? exps)
              ans
              (cases answer ans
                (an-answer (ty1 subst)
                  (let ([new-ans (type-of (car exps) tenv subst)])
                    (loop (cdr exps) new-ans))))))))
      ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
      (const-exp (num)
