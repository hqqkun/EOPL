...
(define tests-for-check
  '(
    ...
    (t0 "list(1,2,3)" (listof int))
    (t1 "let lst = list(1,2,3) in car(lst)" int)
    (t2 "let lst = list(1,2,3) in cdr(lst)" (listof int))
    (t3 "let lst = cons(1, emptylist) in lst" (listof int))
    (t4 "let lst = list(1,2,3) in null?(lst)" bool)
    (t5 "let lst0 = list(1,2,3) in
          let lst1 = cons(zero?(1), lst0) in lst1" error)
    (t6 "let lst = emptylist in car(lst)" tvar1)
    (t7 "let lst = cons(zero?(1), emptylist) in lst" (listof bool))
    ;; simple arithmetic
