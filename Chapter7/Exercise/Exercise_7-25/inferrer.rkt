...
(define type-of
  (lambda (exp tenv subst)
    (cases expression exp
      ...
      ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
      (emptylist-exp ()
        (let ( [item-ty (fresh-tvar-type)])
          (an-answer (list-type item-ty) subst)))
      (null?-exp (exp1)
        (let ([item-ty (fresh-tvar-type)])
          (cases answer (type-of exp1 tenv subst)
            (an-answer (ty1 subst)
              (let ([new-subst (unifier ty1 (list-type item-ty) subst exp1)])
      ...
      (car-exp (exp1)
        (let ([item-ty (fresh-tvar-type)])
          (cases answer (type-of exp1 tenv subst)
            (an-answer (ty1 subst)
              (let ([new-subst (unifier ty1 (list-type item-ty) subst exp1)])
                (an-answer item-ty new-subst))))))
      (cdr-exp (exp1)
        (let ([item-ty (fresh-tvar-type)])
          (cases answer (type-of exp1 tenv subst)
            (an-answer (ty1 subst)
              (let ([new-subst (unifier ty1 (list-type item-ty) subst exp1)])
                (an-answer ty1 new-subst))))))
      (cons-exp (exp1 exp2)
        (cases answer (type-of exp1 tenv subst)
          (an-answer (ty1 subst)
            (cases answer (type-of exp2 tenv subst)
              (an-answer (ty2 subst)
                (let ([new-subst (unifier ty2 (list-type ty1) subst exp2)])
            ...
            (let loop ([exps exps] [subst subst])
              (if (null? exps)
                ...
                (cases answer (type-of (car exps) tenv subst)
                  (an-answer (ty subst)
                    (let ([new-subst (unifier ty1 ty subst (car exps))])
                      (loop (cdr exps) new-subst)))))))))
      ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
