  ...
  (lambda (exp tenv subst)
    (cases expression exp
      ...
      (pair-exp (first second)
        (cases answer (type-of first tenv subst)
          (an-answer (ty1 subst)
            (cases answer (type-of second tenv subst)
              (an-answer (ty2 subst)
                (an-answer
                  (pair-type ty1 ty2)
                  subst))))))
      (unpair-exp (var1 var2 exp1 body)
        (let*
          ( [var1-ty (fresh-tvar-type)]
            [var2-ty (fresh-tvar-type)]
            [new-tenv (extend-tenv var1 var1-ty
              (extend-tenv var2 var2-ty tenv))])
          (cases answer (type-of exp1 tenv subst)
            (an-answer (ty1 subst)
              (let 
                ( [subst (unifier ty1 (pair-type var1-ty var2-ty) subst exp1)])
