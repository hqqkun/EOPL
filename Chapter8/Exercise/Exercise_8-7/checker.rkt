  ...
  (lambda (exp tenv)
    (cases expression exp
      ...
      (qualified-var-exp (m-name var-names)
        (let ( [val (lookup-qualified-var-in-tenv m-name (car var-names) tenv)])
          (if (null? (cdr var-names))
            val
            (let loop ( [var-names (cdr var-names)] [decls (interface-type->decls val)])
              (let ( [val (lookup-variable-name-in-decls (car var-names) decls)])
                (if (null? (cdr var-names))
                  val
                  (loop (cdr var-names)
                    (interface-type->decls val))))))))
