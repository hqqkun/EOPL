...
-(from m take u, from m take v)"
    ...
    ;;! bug test
    ;;! This bug is in runtime, user can use a `var` that is not in the `interface`.
    ;;! But type checker will find this error.
    (bug-test
      "module m1
        interface [u : int]
        body 
