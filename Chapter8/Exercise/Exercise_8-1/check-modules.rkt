...
(define add-module-defns-to-tenv
  (lambda (defns tenv)
    (let ([name-set (mutable-set)])
      (let loop ([defns defns] [tenv tenv])
        (if (null? defns)
          tenv
          (cases module-definition (car defns)
            (a-module-definition (m-name expected-iface m-body)
              (if (set-member? name-set m-name)
                (report-module-name-exists m-name)
                (begin
                  (set-add! name-set m-name)
                  (let ( [actual-iface (interface-of m-body tenv)])
                    (if (<:-iface actual-iface expected-iface tenv)
                      (let
                        ( [new-tenv (extend-tenv-with-module m-name expected-iface tenv)])
                        (loop (cdr defns) new-tenv))
...
(define report-module-name-exists
  (lambda (m-name)
    (eopl:error 'add-module-defns-to-tenv "~s already exists" m-name))
)
